// Originkit "glow-border" (https://originkit.dev), converted from the TSX it
// ships to plain JSX. Changes: types and "use client" dropped, the Framer
// `defaultProps` removed (React 19 ignores them on function components; the
// destructured defaults below already cover every prop), the `live` ref
// written in an effect rather than during render, and the animation loop
...
export default function GlowBorder(props) {
    const {
        mode = DEFAULTS.mode,
    ...
    useEffect(() => {
        ...
        // Reduced motion: keep the static border, never start the loop.
