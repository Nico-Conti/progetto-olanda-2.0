...
import { Info, Flame, TrendingUp, Snowflake } from 'lucide-react';
import TrendBadge from '../common/TrendBadge';
import { getAvg, getTrendData } from '../../utils/stats';

const LeagueTrends = ({ stats, onTeamClick }) => {
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left text-zinc-300">
                        ...
                        <tbody className="divide-y divide-white/5 text-sm">
                            {teams.map(team => {
                                const hTrend = getTrendData(stats[team].home_corners_total, nGames);
                                const aTrend = getTrendData(stats[team].away_corners_total, nGames);

                                const hForAvg = getAvg(stats[team].home_corners_for);
                                const hAgAvg = getAvg(stats[team].home_corners_ag);
                                const aForAvg = getAvg(stats[team].away_corners_for);
                                const aAgAvg = getAvg(stats[team].away_corners_ag);

                                return (
                                    <tr key={team} className="hover:bg-white/[0.03] transition-colors group">
                                        <td className="px-5 py-3 font-bold text-white text-base border-r border-white/5">
                                            <button
                                                onClick={() => onTeamClick && onTeamClick(team)}
                                                className="hover:text-emerald-400 transition-colors text-left w-full"
                                            >
                                                {team}
                                            </button>
                                        </td>
